...
// Experimental F16-specific dot helper for mul_mv _4 path.
// Keep the algorithm unchanged while forcing a more explicit half4 -> float4 lowering.
template<typename T04, typename T14>
static inline float ggml_mul_mv_dot4(device const T04 & a, thread const T14 & b) {
    return dot(float4(a), float4(b));
}

static inline float ggml_mul_mv_dot4(device const half4 & a, thread const half4 & b) {
    const float4 af = float4(float(a.x), float(a.y), float(a.z), float(a.w));
    const float4 bf = float4(float(b.x), float(b.y), float(b.z), float(b.w));
    return dot(af, bf);
}
    ...
    for (int ib = ib0; ib < nb; ib += NSG*NF) {
        ...
        for (short row = 0; row < NR0; row++) {
            ...
            FOR_UNROLL (short i = 0; i < NF4; ++i) {
                sumq += ggml_mul_mv_dot4(xb4[i], yl4[i]);
            }

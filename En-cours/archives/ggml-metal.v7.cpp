...
static bool ggml_backend_metal_device_offload_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    // MGPU PP dense MUL_MAT guard:
    // for dense non-decode MUL_MAT, inter-GPU offload can cost more than it saves
    // around small / medium PP batch sizes. Keep this as an explicit knob so dense
    // models (e.g. Qwen3 14B F16) can be tuned without affecting MoE MUL_MAT_ID.
    if (is_mgpu &&
        ...
        !is_decode_like &&
        !small_mat_override &&
        pp_dense_mul_mat_min_bs >= 0) {
        if (effective_offload_min_bs < pp_dense_mul_mat_min_bs) {
            effective_offload_min_bs = pp_dense_mul_mat_min_bs;
            pp_dense_guard = true;
        }
    }
    ...
    if (ggml_metal_env_offload_debug_()) {
        fprintf(stderr,
            "ggml-metal offload-threshold: dev='%s' op=%s tensor='%s' bs=%lld offload_min_bs=%lld effective_offload_min_bs=%lld is_mgpu=%d is_decode_like=%d is_decode_strict=%d is_decode_small_mat=%d is_pp_small_batch=%d aux_batch=%lld small_mat_offload_max_bs=%d small_mat_override=%d pp_dense_mul_mat_min_bs=%d pp_dense_guard=%d\n",
    ...
    if (bs < effective_offload_min_bs) {
        ggml_metal_log_offload_decision_(
            dev, op, bs, false,
            pp_dense_guard ? "mgpu-pp-dense-mul_mat-min-bs" : "device-offload-min-bs");
        return false;
    }

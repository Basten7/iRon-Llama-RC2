...
static bool ggml_metal_env_decode_stats_(void) {
    const char * v = getenv("GGML_METAL_MGPU_DECODE_STATS");
    return v != NULL && v[0] != '\0' && strcmp(v, "0") != 0;
}

struct ggml_metal_decode_stats_ {
    std::atomic<long long> decode_candidates{0};
    std::atomic<long long> decode_mul_mat{0};
    std::atomic<long long> decode_mul_mat_id{0};
    std::atomic<long long> decode_strict{0};
    std::atomic<long long> decode_small_mat{0};
    std::atomic<long long> decode_generic{0};
    std::atomic<long long> offloaded{0};
    std::atomic<long long> rejected_decode_guard{0};
    std::atomic<long long> rejected_device_min_bs{0};
    std::atomic<long long> rejected_other{0};
    std::atomic<long long> copy_calls{0};
    std::atomic<long long> copy_bytes{0};
};

static ggml_metal_decode_stats_ g_ggml_metal_decode_stats_;
static std::atomic<bool> g_ggml_metal_decode_stats_logged_(false);

static inline bool ggml_metal_is_decode_candidate_(
        bool is_mgpu,
        const ggml_tensor * op,
    ...
    return cls.is_decode_like || cls.is_decode_strict || cls.is_decode_small_mat;
}

static void ggml_metal_record_decode_decision_(
        const ggml_tensor * op,
        const ggml_metal_decode_shape_class_ & cls,
        bool is_mgpu,
        bool decision,
        const char * reason) {
    if (!ggml_metal_env_decode_stats_()) {
        return;
    }
    if (!ggml_metal_is_decode_candidate_(is_mgpu, op, cls)) {
        return;
    }

    g_ggml_metal_decode_stats_.decode_candidates.fetch_add(1, std::memory_order_relaxed);
    if (op->op == GGML_OP_MUL_MAT) {
        g_ggml_metal_decode_stats_.decode_mul_mat.fetch_add(1, std::memory_order_relaxed);
    } else if (op->op == GGML_OP_MUL_MAT_ID) {
        g_ggml_metal_decode_stats_.decode_mul_mat_id.fetch_add(1, std::memory_order_relaxed);
    }

    if (cls.is_decode_strict) {
        g_ggml_metal_decode_stats_.decode_strict.fetch_add(1, std::memory_order_relaxed);
    } else if (cls.is_decode_small_mat) {
        g_ggml_metal_decode_stats_.decode_small_mat.fetch_add(1, std::memory_order_relaxed);
    } else {
        g_ggml_metal_decode_stats_.decode_generic.fetch_add(1, std::memory_order_relaxed);
    }

    if (decision) {
        g_ggml_metal_decode_stats_.offloaded.fetch_add(1, std::memory_order_relaxed);
        return;
    }

    const char * r = reason ? reason : "";
    if (strstr(r, "decode-guard") != NULL) {
        g_ggml_metal_decode_stats_.rejected_decode_guard.fetch_add(1, std::memory_order_relaxed);
    } else if (strcmp(r, "device-offload-min-bs") == 0) {
        g_ggml_metal_decode_stats_.rejected_device_min_bs.fetch_add(1, std::memory_order_relaxed);
    } else {
        g_ggml_metal_decode_stats_.rejected_other.fetch_add(1, std::memory_order_relaxed);
    }
}

static void ggml_metal_record_decode_copy_(
    ...
    g_ggml_metal_decode_stats_.copy_calls.fetch_add(1, std::memory_order_relaxed);
    g_ggml_metal_decode_stats_.copy_bytes.fetch_add((long long) nbytes, std::memory_order_relaxed);
}

static void ggml_metal_log_decode_stats_summary_(void) {
    if (!ggml_metal_env_decode_stats_()) {
        return;
    }
    bool expected = false;
    if (!g_ggml_metal_decode_stats_logged_.compare_exchange_strong(expected, std::memory_order_relaxed)) {
        return;
    }

    fprintf(stderr,
        "ggml-metal: decode-stats: candidates=%lld mul_mat=%lld mul_mat_id=%lld strict=%lld small_mat=%lld generic=%lld offloaded=%lld rejected_decode_guard=%lld rejected_device_min_bs=%lld rejected_other=%lld metal_copy_calls=%lld metal_copy_bytes=%lld\n",
        (long long) g_ggml_metal_decode_stats_.decode_candidates.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.decode_mul_mat.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.decode_mul_mat_id.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.decode_strict.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.decode_small_mat.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.decode_generic.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.offloaded.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.rejected_decode_guard.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.rejected_device_min_bs.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.rejected_other.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.copy_calls.load(std::memory_order_relaxed),
        (long long) g_ggml_metal_decode_stats_.copy_bytes.load(std::memory_order_relaxed));
}
...
static void ggml_backend_metal_free(ggml_backend_t backend) {
    ...
    ggml_metal_free(ctx);
    ggml_metal_log_decode_stats_summary_();
...
static bool ggml_backend_metal_cpy_tensor_async(ggml_backend_t backend_src, ggml_backend_t backend_dst, const ggml_tensor * src, ggml_tensor * dst) {
    ...
    ggml_metal_record_decode_copy_(backend_src, backend_dst, nbytes);
    if (nbytes == 0) {
        return true;
    }
...
static bool ggml_backend_metal_device_offload_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    if (op->op != GGML_OP_MUL_MAT && op->op != GGML_OP_MUL_MAT_ID) {
        ...
        ggml_metal_record_decode_decision_(op, cls, is_mgpu, false, "unsupported-op-class");
        return false;
    }
    ...
    if (!is_mgpu) {
        ...
        ggml_metal_record_decode_decision_(op, cls, is_mgpu, ok, reason);
        return ok;
    }
    ...
    if (cls.is_decode_strict) {
        ...
        ggml_metal_record_decode_decision_(op, cls, is_mgpu, false, reason);
        return false;
    }
    ...
    if (bs < effective_offload_min_bs) {
        ...
        ggml_metal_record_decode_decision_(op, cls, is_mgpu, false, "device-offload-min-bs");
        return false;
    }
    ...
    ggml_metal_record_decode_decision_(op, cls, is_mgpu, true, reason);

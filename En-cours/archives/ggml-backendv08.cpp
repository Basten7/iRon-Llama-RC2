...
static inline int ggml_backend_sched_env_moe_mul_mat_id_weight_reuse_min_inputs_() {
    const char * v = getenv("GGML_SCHED_MOE_MUL_MAT_ID_WEIGHT_REUSE_MIN_INPUTS");
    if (v == NULL || v[0] == '\0') {
        return -1;
    }
...
static inline bool ggml_backend_sched_can_delay_weight_split_for_moe_mul_mat_id_(
        const struct ggml_tensor * node,
        int split_n_inputs) {
    const int min_inputs = ggml_backend_sched_env_moe_mul_mat_id_weight_reuse_min_inputs_();
    if (min_inputs <= 0 || node == NULL || node->op != GGML_OP_MUL_MAT_ID) {
        return false;
    }

    const int64_t bs        = node->ne[2];
    const int64_t aux_batch = node->ne[3];
    const bool is_decode_like = (bs <= 1);

    // Keep decode-like and tiny PP on the conservative path.
    // For MoE, allow medium/large PP batches to stay together a bit longer so that
    // consecutive MUL_MAT_ID nodes can reuse the same sparse-expert copy phase.
    if (is_decode_like || bs < 8) {
        return false;
    }
                    ...
                    if (src->buffer != NULL && src->buffer->usage == GGML_BACKEND_BUFFER_USAGE_WEIGHTS) {
                        ...
                        if (src_backend_id != cur_backend_id && !ggml_backend_sched_buffer_supported(sched, src, cur_backend_id)) {
                            if (ggml_backend_sched_can_delay_weight_split_for_dense_mul_mat_(node, split->n_inputs) ||
                                ggml_backend_sched_can_delay_weight_split_for_moe_mul_mat_id_(node, split->n_inputs)) {
                                continue;
                            }
                            need_new_split = true;
                            break;

...
static int ggml_metal_decode_q80_shape_threshold_default_(void) {
    return 2048;
}

static int ggml_metal_env_decode_q80_shape_threshold_(void) {
    return ggml_metal_env_int("GGML_METAL_DECODE_Q80_SHAPE_THRESHOLD", ggml_metal_decode_q80_shape_threshold_default_());
}

static bool ggml_metal_has_explicit_decode_q80_env_(void) {
    const char * nr0 = getenv("GGML_METAL_DECODE_Q80_NR0");
    const char * nsg = getenv("GGML_METAL_DECODE_Q80_NSG");
    return (nr0 && nr0[0]) || (nsg && nsg[0]);
}

static bool ggml_metal_apply_decode_q80_shape_heuristic_(
    ...
    if (ggml_metal_env_mmv_dec_nr0() > 0 || ggml_metal_env_mmv_dec_nsg() > 0) {
        return false;
    }

    if (ggml_metal_has_explicit_decode_q80_env_()) {
        return false;
    }

    const int threshold = ggml_metal_env_decode_q80_shape_threshold_();
    if (threshold <= 0) {
        return false;
    }
    ...
    if (ne00 <= threshold) {
        *nr0 = 4;
        *nsg = 2;
        *reason = cls.is_decode_strict ? "decode-q80-shape-heuristic" : "decode-q80-small-mat-heuristic";
        return true;
    }
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        if (op->src[0]->type == GGML_TYPE_MXFP4) {
            const char * env = getenv("GGML_METAL_MMV_EXT_PP_MXFP4_NSG");
            if (env && env[0]) {
        ...
        if (op->src[0]->type == GGML_TYPE_MXFP4) {
            const char * env = getenv("GGML_METAL_MMV_EXT_PP_MXFP4_NXPSG");
            if (env && env[0]) {
                const int v = atoi(env);
                if (v == 4 || v == 8 || v == 16) nxpsg = v;
            }
        }
        ...
        if (op->src[0]->type == GGML_TYPE_MXFP4) {
            const char * env = getenv("GGML_METAL_MMV_EXT_PP_MXFP4_R1PTG");
            if (env && env[0]) {
                const int v = atoi(env);
                if (v >= 1 && v <= 8) r1ptg = v;
            }
        ...
        props_dev->has_simdgroup_mm && props_dev->supports_gpu_family_apple7 &&
        ...
        const bool use_dec_path = cls.is_decode_strict || cls.is_decode_small_mat;
        const bool use_generic_dec_override = use_dec_path && (ggml_metal_env_mmv_dec_nr0() > 0 || ggml_metal_env_mmv_dec_nsg() > 0);
        const char * select_reason = nullptr;

        if (use_generic_dec_override) {
            ...
            select_reason = cls.is_decode_strict ? "decode-strict-override" : "decode-small-mat-override";
        } else if (ggml_metal_apply_decode_q80_shape_heuristic_(op, cls, &nr0, &nsg, &select_reason)) {
            // q8_0 decode heuristic applied
            ...
            select_reason = "pipeline-default";
        }
...
int ggml_metal_op_mul_mat_id(ggml_metal_op_t ctx, int idx) {
    ...
    if (props_dev->has_simdgroup_mm && props_dev->supports_gpu_family_apple7 &&
        ne00 >= 64 && (ne21 >= ne21_mm_id_min)) {        // some Metal matrix data types require aligned pointers
        // ref: https://developer.apple.com/metal/Metal-Shading-Language-Specification.pdf (Table 2.5)
        //switch (op->src[0]->type) {
        //    case GGML_TYPE_F32:  GGML_ASSERT(nb01 % 16 == 0); break;
        //    case GGML_TYPE_F16:  GGML_ASSERT(nb01 % 8  == 0); break;

...
static int ggml_metal_env_decode_bf16_shape_threshold(void) {
    static int cached = INT_MIN;
    if (cached == INT_MIN) cached = ggml_metal_env_int("GGML_METAL_DECODE_BF16_SHAPE_THRESHOLD", 2048);
    return cached;
}
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        if (use_dec_override) {
            ...
            const bool has_env_override = (env_nr0 > 0) || (env_nsg > 0);
            if (has_env_override) {
            ...
            } else if (op->src[0]->type == GGML_TYPE_BF16 && cls.is_decode_strict) {
                const int bf16_shape_threshold = ggml_metal_env_decode_bf16_shape_threshold();
                if (bf16_shape_threshold > 0 && ne00 <= bf16_shape_threshold) {
                    nr0 = 4;
                    nsg = 2;
                    select_reason = "decode-bf16-shape-small";
                } else {
                    select_reason = "decode-strict-default";
                }
            } else {
                select_reason = cls.is_decode_strict ? "decode-strict-default" : "decode-small-mat-default";
            }
        }

...
struct ggml_metal_mgpu_policy_stats_ {
    uint64_t mul_mat_gpu = 0;
    uint64_t mul_mat_cpu = 0;
    uint64_t mul_mat_id_gpu = 0;
    uint64_t mul_mat_id_cpu = 0;
    std::unordered_map<std::string, uint64_t> ops_by_dev;
...
static void ggml_metal_mgpu_policy_dump_summary_(void) {
    ...
    fprintf(stderr,
            "ggml-metal policy-summary: MUL_MAT gpu=%llu cpu=%llu | MUL_MAT_ID gpu=%llu cpu=%llu\n",
            (unsigned long long) g_ggml_metal_mgpu_policy_stats_.mul_mat_gpu,
            (unsigned long long) g_ggml_metal_mgpu_policy_stats_.mul_mat_cpu,
            (unsigned long long) g_ggml_metal_mgpu_policy_stats_.mul_mat_id_gpu,
            (unsigned long long) g_ggml_metal_mgpu_policy_stats_.mul_mat_id_cpu);
    if (!g_ggml_metal_mgpu_policy_stats_.ops_by_dev.empty()) {
        fprintf(stderr, "ggml-metal policy-summary-dev:");
        for (const auto & kv : g_ggml_metal_mgpu_policy_stats_.ops_by_dev) {
...
static void ggml_metal_mgpu_policy_record_(
    ...
    const char * dev_name = ggml_backend_dev_description(dev);
    std::string dev_key = decision_gpu
        ? std::string(dev_name ? dev_name : "(unknown)")
        : std::string("CPU");

    {
        ...
        switch (op->op) {
            case GGML_OP_MUL_MAT:
                if (decision_gpu) g_ggml_metal_mgpu_policy_stats_.mul_mat_gpu++;
                else              g_ggml_metal_mgpu_policy_stats_.mul_mat_cpu++;
                break;
            case GGML_OP_MUL_MAT_ID:
                if (decision_gpu) g_ggml_metal_mgpu_policy_stats_.mul_mat_id_gpu++;
                else              g_ggml_metal_mgpu_policy_stats_.mul_mat_id_cpu++;
                break;
            default:
                break;
        }
        g_ggml_metal_mgpu_policy_stats_.ops_by_dev[dev_key]++;
    }

    if (ggml_metal_env_mgpu_policy_log_()) {
        fprintf(stderr,
                "ggml-metal policy: dev='%s' op=%s tensor='%s' bs=%lld ne=[%lld,%lld,%lld,%lld] decode_like=%d decode_strict=%d decode_small_mat=%d pp_small_batch=%d aux_batch=%lld decision=%s reason=%s\n",
                dev_name ? dev_name : "(unknown)",
...
static bool ggml_backend_metal_device_offload_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    if (op->op != GGML_OP_MUL_MAT && op->op != GGML_OP_MUL_MAT_ID) {
        ...
        ggml_metal_mgpu_policy_record_(dev, op, bs, cls, false, "unsupported-op-class");
        return false;
    }
    ...
    if (!is_mgpu) {
        ...
        ggml_metal_mgpu_policy_record_(dev, op, bs, cls, ok, reason);
        return ok;
    }
    ...
    if (cls.is_decode_strict) {
        ...
        ggml_metal_mgpu_policy_record_(dev, op, bs, cls, false, reason);
        return false;
    }
    ...
    if (bs < effective_offload_min_bs) {
        ...
        ggml_metal_mgpu_policy_record_(dev, op, bs, cls, false, "device-offload-min-bs");
        return false;
    }
    ...
    ggml_metal_mgpu_policy_record_(dev, op, bs, cls, true, reason);

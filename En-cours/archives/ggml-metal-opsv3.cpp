...
static bool ggml_metal_env_mmv_ext_pp_bf16_enable(void) {
    static int cached = -2;
    if (cached == -2) cached = ggml_metal_env_tri("GGML_METAL_MMV_EXT_PP_BF16_ENABLE", 0);
    return cached == 1;
}
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    // On AMD, this path is the practical dense-PP tuning lever because MUL_MM stays Apple-only.
    // BF16 is opt-in to avoid selecting a kernel family that may not exist on every tree.
    const bool allow_bf16_ext = ggml_metal_env_mmv_ext_pp_bf16_enable();
    const bool dense_ext_type_ok =
        op->src[0]->type == GGML_TYPE_F32 ||
        op->src[0]->type == GGML_TYPE_F16 ||
        (op->src[0]->type == GGML_TYPE_BF16 && allow_bf16_ext);
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        ggml_metal_log_mmv_ext_select_(
            ...
            r1ptg,
            use_ext_pp_override
                ? (op->src[0]->type == GGML_TYPE_BF16 ? "pp-small-batch-override-bf16-ext" : "pp-small-batch-override")
                : (op->src[0]->type == GGML_TYPE_BF16 ? "bf16-ext-opt-in" : "pipeline-default"));

...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    struct ggml_backend_sched_moe_ids_cache_range {
        int32_t first_id;
        int32_t last_id;
    };

    struct ggml_backend_sched_moe_ids_cache_entry {
        ggml_tensor * ids_tensor;
        int64_t n_expert;
        std::vector<ggml_bitset_t> used_ids;
        std::vector<ggml_backend_sched_moe_ids_cache_range> expert_ranges;
    };

    std::vector<ggml_backend_sched_moe_ids_cache_entry> moe_ids_cache;
    std::vector<int32_t> moe_ids_tmp;

    auto ggml_backend_sched_get_moe_ids_cache_ =
            [&](ggml_tensor * ids_tensor, ggml_backend_t ids_backend, const int64_t n_expert)
            -> ggml_backend_sched_moe_ids_cache_entry * {
        for (auto & entry : moe_ids_cache) {
            if (entry.ids_tensor == ids_tensor && entry.n_expert == n_expert) {
                return &entry;
            }
        }

        ggml_backend_sched_moe_ids_cache_entry entry = {
        ...
        moe_ids_tmp.resize(ggml_nbytes(ids_tensor) / sizeof(int32_t));
        ggml_backend_tensor_get_async(ids_backend, ids_tensor, moe_ids_tmp.data(), 0, ggml_nbytes(ids_tensor));
        ggml_backend_synchronize(ids_backend);

        entry.used_ids.resize(ggml_bitset_size(n_expert));
        for (int64_t i1 = 0; i1 < ids_tensor->ne[1]; i1++) {
            for (int64_t i0 = 0; i0 < ids_tensor->ne[0]; i0++) {
                const int32_t id = moe_ids_tmp[i1 * ids_tensor->nb[1]/sizeof(int32_t) + i0 * ids_tensor->nb[0]/sizeof(int32_t)];
                GGML_ASSERT(id >= 0 && id < n_expert);
                ggml_bitset_set(entry.used_ids.data(), id);
            }
        }

        int32_t id = 0;
        while (id < n_expert && !ggml_bitset_get(entry.used_ids.data(), id)) {
            id++;
        }

        if (id < n_expert) {
            ggml_backend_sched_moe_ids_cache_range range = { id, id };

            for (++id; id < n_expert; ++id) {
                if (!ggml_bitset_get(entry.used_ids.data(), id)) {
                    continue;
                }

                if (id == range.last_id + 1) {
                    range.last_id = id;
                    continue;
                }

                entry.expert_ranges.push_back(range);
                range.first_id = id;
                range.last_id = id;
            }
        ...
        moe_ids_cache.emplace_back(std::move(entry));
        return &moe_ids_cache.back();
    };
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                if (split->graph.n_nodes > 0 &&
                    ...
                    ggml_backend_sched_moe_ids_cache_entry * ids_cache =
                        ggml_backend_sched_get_moe_ids_cache_(ids_tensor, ids_backend, n_expert);
                    GGML_ASSERT(ids_cache != nullptr);
                    ...
                    for (const auto & range : ids_cache->expert_ranges) {
                        copy_experts(range.first_id, range.last_id);
                    }
                } else {

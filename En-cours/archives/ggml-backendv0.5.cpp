// Note: porting this file to C++ is a work in progress ggml-backendv0.5.cpp ciblé sur 3/4 GPU,
...
static int ggml_backend_sched_env_i32_default_(const char * name, int defv) {
...
static int64_t ggml_backend_sched_env_i64_default_(const char * name, int64_t defv) {
    const char * v = getenv(name);
    if (v == NULL || v[0] == '\0') {
        return defv;
    }
...
static bool ggml_backend_sched_is_small_path_(const ggml_backend_sched_decode_shape_class_ & cls) {
    return cls.is_decode_small_mat || cls.is_pp_small_batch;
}

static bool ggml_backend_sched_is_large_path_(const ggml_backend_sched_decode_shape_class_ & cls) {
    return !cls.is_decode_like && !ggml_backend_sched_is_small_path_(cls);
}

static int64_t ggml_backend_sched_estimate_matmul_work_(const ggml_tensor * op, const ggml_backend_sched_decode_shape_class_ & cls) {
...
static bool ggml_backend_sched_is_metal_backend_(ggml_backend_t backend) {
    if (backend == NULL) {
        return false;
    }

    const char * name = ggml_backend_name(backend);
    return name != NULL && strncmp(name, "Metal", 5) == 0;
}

static int ggml_backend_sched_metal_backend_count_(ggml_backend_sched_t sched) {
    int count = 0;
    for (int i = 0; i < sched->n_backends; ++i) {
...
static int ggml_backend_sched_active_metal_cap_(ggml_backend_sched_t sched, const ggml_tensor * node) {
    ...
    const int n_metal_backends = ggml_backend_sched_metal_backend_count_(sched);
    if (n_metal_backends <= 1) {
        return 0;
    }

    const ggml_backend_sched_decode_shape_class_ cls = ggml_backend_sched_classify_decode_shape_(node);
    int n_active = n_metal_backends;

    if (cls.is_decode_like || cls.is_decode_small_mat) {
        const int cap = ggml_backend_sched_env_i32_default_("GGML_METAL_MGPU_MAX_ACTIVE_DEVS_DECODE", 0);
        if (cap > 0 && n_active > cap) {
            n_active = cap;
        }
    } else if (cls.is_pp_small_batch) {
        const int cap = ggml_backend_sched_env_i32_default_("GGML_METAL_MGPU_MAX_ACTIVE_DEVS_SMALL", 0);
        if (cap > 0 && n_active > cap) {
            n_active = cap;
        }
    } else if (ggml_backend_sched_is_large_path_(cls)) {
        // Large-path policy V0.5:
        // - keep explicit MAX_ACTIVE_DEVS_LARGE as an override
        // - otherwise be more conservative when 3/4+ Metal backends are visible
        // - preserve the 2-GPU case as-is
        const int explicit_cap = ggml_backend_sched_env_i32_default_("GGML_METAL_MGPU_MAX_ACTIVE_DEVS_LARGE", 0);
        if (explicit_cap > 0 && n_active > explicit_cap) {
            n_active = explicit_cap;
        }

        int64_t min_work = ggml_backend_sched_env_i64_default_("GGML_METAL_MGPU_MIN_WORK_PER_GPU_LARGE", 0);
        const int64_t work_total = ggml_backend_sched_estimate_matmul_work_(node, cls);

        if (min_work <= 0 && n_metal_backends >= 3) {
            // Default conservative threshold tuned for 3/4 GPU prefill on AMD dGPU:
            // use a higher threshold as the number of visible Metal backends grows.
            min_work = 134217728LL; // 128M work units baseline
            if (node->src[0] != NULL) {
            ...
            if (n_metal_backends >= 4) {
                min_work *= 2;
            } else { // n_metal_backends == 3
                min_work = (min_work * 3) / 2;
            }
        }

        if (min_work > 0) {
            // For large-path we never auto-shrink below 2 devices unless the user
            // explicitly asked for it via MAX_ACTIVE_DEVS_LARGE.
            const int auto_floor = explicit_cap == 1 ? 1 : 2;
            while (n_active > auto_floor && work_total / n_active < min_work) {
                --n_active;
            }
        }

        // Optional hard auto-cap for 3+ visible Metal backends.
        // 0 = disabled. Useful to force a 2-GPU large-path policy on 4-GPU runs.
        if (n_metal_backends >= 3) {
            const int auto_cap_3plus = ggml_backend_sched_env_i32_default_("GGML_METAL_MGPU_MAX_ACTIVE_DEVS_LARGE_3PLUS", 0);
            if (auto_cap_3plus > 0 && n_active > auto_cap_3plus) {
                n_active = auto_cap_3plus;
            }
        }
    }

    if (n_active <= 0 || n_active >= n_metal_backends) {
        return 0;
    }

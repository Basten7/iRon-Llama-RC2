...
static inline bool ggml_backend_metal_copy_use_tensor_io_(void) {
    const char * v = getenv("GGML_METAL_MGPU_COPY_TENSOR_IO");
    if (v != NULL && v[0] != '\0') {
        return atoi(v) != 0;
    }

    // Default this experiment to MGPU only.
    return ggml_metal_env_is_mgpu_enabled_();
}
...
static bool ggml_backend_metal_cpy_tensor_async(ggml_backend_t backend_src, ggml_backend_t backend_dst, const ggml_tensor * src, ggml_tensor * dst) {
    ...
    if (ggml_backend_metal_copy_use_tensor_io_()) {
        if (src_is_metal) {
            ggml_backend_metal_trace_copy_("cpy.src-blocking", backend_src, backend_dst, src, dst, nbytes, "src-metal-to-host");
        }
        ggml_backend_tensor_get(src, tmp.data(), 0, nbytes);

        if (dst_is_metal) {
            ggml_backend_metal_trace_copy_("cpy.dst-blocking", backend_src, backend_dst, src, dst, nbytes, "host-to-dst-metal");
        }
        ggml_backend_tensor_set(dst, tmp.data(), 0, nbytes);

        ggml_backend_metal_trace_copy_("cpy.end", backend_src, backend_dst, src, dst, nbytes, "ok-blocking-tensor-io");
        return true;
    }
...
static void ggml_backend_metal_event_record(ggml_backend_t backend, ggml_backend_event_t event) {
    GGML_ASSERT(backend);
    GGML_ASSERT(event);
    ggml_metal_device_t ctx_dev = (ggml_metal_device_t) backend->device->context;
    ggml_metal_shared_event_t ev = (ggml_metal_shared_event_t) event->context;
    GGML_ASSERT(ev != NULL);
    GGML_ASSERT(ggml_metal_device_supports_shared_events(ctx_dev));
    const bool ok = ggml_metal_shared_event_record(ev, ctx_dev);
    GGML_ASSERT(ok);
}
...
static void ggml_backend_metal_event_wait(ggml_backend_t backend, ggml_backend_event_t event) {
    ...
    GGML_ASSERT(ev != NULL);
    GGML_ASSERT(ggml_metal_device_supports_shared_events(ctx_dev));
    const bool ok = ggml_metal_shared_event_wait(ev, ctx_dev);
    GGML_ASSERT(ok);
}
...
static void ggml_backend_metal_device_get_props(ggml_backend_dev_t dev, ggml_backend_dev_props * props) {
    ...
    const bool has_native_events = ggml_metal_device_supports_shared_events((ggml_metal_device_t) dev->context);

    props->caps = {
        // Native Metal shared events preserve async semantics safely in MGPU.
        // If shared events are unavailable, keep the old conservative fallback.
        /* .async                 = */ has_native_events ? true : !is_mgpu,
        /* .host_buffer           = */ false,
        /* .buffer_from_host_ptr  = */ true,
        /* .events                = */ has_native_events,
    };
}
...
static ggml_backend_event_t ggml_backend_metal_device_event_new(ggml_backend_dev_t dev) {
    ggml_metal_device_t ctx_dev = (ggml_metal_device_t) dev->context;
    if (!ggml_metal_device_supports_shared_events(ctx_dev)) return NULL;
    ggml_metal_shared_event_t ev_ctx = ggml_metal_shared_event_new(ctx_dev);
    if (ev_ctx == NULL) return NULL;
    ggml_backend_event_t event = (ggml_backend_event_t) calloc(1, sizeof(*event));
    if (event == NULL) { ggml_metal_shared_event_free(ev_ctx); return NULL; }
    event->device = dev;
    event->context = ev_ctx;
    return event;
}
...
static void ggml_backend_metal_device_event_free(ggml_backend_dev_t dev, ggml_backend_event_t event) {
    ...
    ggml_metal_shared_event_t ev_ctx = (ggml_metal_shared_event_t) event->context;
    ggml_metal_shared_event_free(ev_ctx);
    free(event);
}
...
static void ggml_backend_metal_device_event_synchronize(ggml_backend_dev_t dev, ggml_backend_event_t event) {
    ...
    GGML_ASSERT(event);
    ggml_metal_shared_event_t ev_ctx = (ggml_metal_shared_event_t) event->context;
    GGML_ASSERT(ev_ctx != NULL);
    const bool ok = ggml_metal_shared_event_synchronize(ev_ctx);
    GGML_ASSERT(ok);
}

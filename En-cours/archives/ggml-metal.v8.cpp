...
static bool ggml_backend_metal_device_offload_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    long double   dense_mul_mat_work_m = -1.0L;
    ...
    // MGPU dense PP guard:
    // for dense MUL_MAT in prompt-processing-like shapes, gate offload on an
    // approximate total work threshold rather than bs alone. This helps avoid
    // shipping medium dense matmuls across GPUs when copy/sync overhead is not
    // amortized.
    if (is_mgpu &&
        ...
        !cls.is_decode_small_mat &&
        !small_mat_override &&
        pp_dense_mul_mat_min_work_m >= 0 &&
        op->src[0] != NULL) {
        const int64_t k = op->src[0]->ne[0];
        const int64_t n = op->ne[0];
        const int64_t m_total = bs * std::max<int64_t>(1, cls.aux_batch);
        dense_mul_mat_work_m = ((long double) k * (long double) n * (long double) m_total) / 1000000.0L;

        if (dense_mul_mat_work_m < (long double) pp_dense_mul_mat_min_work_m) {
            ggml_metal_log_offload_decision_(dev, op, bs, false, "mgpu-pp-dense-mul_mat-work-guard");
            return false;
        }
    }
    ...
    if (ggml_metal_env_offload_debug_()) {
        fprintf(stderr,
            "ggml-metal offload-threshold: dev='%s' op=%s tensor='%s' bs=%lld offload_min_bs=%lld effective_offload_min_bs=%lld is_mgpu=%d is_decode_like=%d is_decode_strict=%d is_decode_small_mat=%d is_pp_small_batch=%d aux_batch=%lld small_mat_offload_max_bs=%d small_mat_override=%d pp_dense_mul_mat_min_work_m=%d dense_mul_mat_work_m=%.2Lf\n",

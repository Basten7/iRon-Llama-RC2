// Note: porting this file to C++ is a work in progress ggml-backendv0.4.1 large Path + Trace
...
static inline int64_t ggml_backend_sched_env_i64_default_(const char * name, int64_t defv) {
    const char * s = getenv(name);
    if (s == NULL || s[0] == '\0') {
        return defv;
    }
...
static inline bool ggml_backend_sched_is_small_path_(const ggml_backend_sched_decode_shape_class_ & cls) {
    return cls.is_decode_small_mat || cls.is_pp_small_batch;
}

static inline bool ggml_backend_sched_is_large_path_(const ggml_backend_sched_decode_shape_class_ & cls) {
    return !cls.is_decode_like && !ggml_backend_sched_is_small_path_(cls);
}

static inline int64_t ggml_backend_sched_estimate_matmul_work_(const ggml_tensor * op, const ggml_backend_sched_decode_shape_class_ & cls) {
...
static inline int ggml_backend_sched_metal_backend_count_(ggml_backend_sched_t sched) {
    int count = 0;
    for (int i = 0; i < sched->n_backends; ++i) {
...
static int ggml_backend_sched_active_metal_cap_(ggml_backend_sched_t sched, const ggml_tensor * node) {
    ...
    if (cls.is_decode_like || cls.is_decode_small_mat) {
        ...
        const int cap = ggml_backend_sched_env_i32_default_("GGML_METAL_MGPU_MAX_ACTIVE_DEVS_LARGE", 0);
        if (cap > 0 && n_active > cap) {
            n_active = cap;
        }

        const int64_t min_work = ggml_backend_sched_env_i64_default_("GGML_METAL_MGPU_MIN_WORK_PER_GPU_LARGE", 0);
        if (min_work > 0) {
            const int64_t work_total = ggml_backend_sched_estimate_matmul_work_(node, cls);
            while (n_active > 1 && work_total / n_active < min_work) {
                --n_active;
            }
        }
    }
